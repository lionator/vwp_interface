  ...
  vehicle_info_(vehicle_info_util::VehicleInfoUtil(*this).getVehicleInfo())
{
  ...
  velocity_report_publisher_ = create_publisher<autoware_auto_vehicle_msgs::msg::VelocityReport>(
    "/vehicle/status/velocity_status", 1);
  steering_report_publisher_ = create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>(
  ...
  vehicle_data_subscriber_ = create_subscription<vwp_msgs::msg::VehicleData>(
    "/vwp/vehicle_can/vehicle_data", rclcpp::QoS{1},
    std::bind(&VehicleStatusPublisher::callbackVehicleData, this, std::placeholders::_1));

  position_data_subscriber_ = create_subscription<vwp_msgs::msg::PositionDataVehicleCan>(
    "/vwp/vehicle_can/position_data", rclcpp::QoS{1},
    std::bind(&VehicleStatusPublisher::callbackPositionData, this, std::placeholders::_1));
}

void VehicleStatusPublisher::callbackVehicleData(
  const vwp_msgs::msg::VehicleData::ConstSharedPtr vehicle_data_msg_ptr)
{
  currentData.vehicleVelocity = vehicle_data_msg_ptr->vehicle_velocity_ms;

  currentData.steeringWheelAngleRad = vehicle_data_msg_ptr->steering_wheel_angle_rad;

  currentData.wheel_speed_rl_ms = vehicle_data_msg_ptr->wheel_speed_rl_ms;
  currentData.wheel_speed_rr_ms = vehicle_data_msg_ptr->wheel_speed_rr_ms;
}

void VehicleStatusPublisher::callbackPositionData(
  const vwp_msgs::msg::PositionDataVehicleCan::ConstSharedPtr position_data_msg_ptr)
{
...
void VehicleStatusPublisher::publish()
{
  const double current_velocity =
    currentData.vehicleVelocity /
    3.6f;  // calculateVehicleVelocity();  // current vehicle speed > 0 [m/s]
  const double current_steer_wheel =
    currentData.steeringWheelAngleRad;  // current vehicle steering wheel angle [rad]
  ...
  /* publish velocity report */
  {
    ...
    velocity_report_msg_.heading_rate =
...
double VehicleStatusPublisher::calculateVariableGearRatio(
